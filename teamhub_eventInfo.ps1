cls

##後程config持ちにする
#ログインURL --ログインセッション確立の為に必須
$loginUrl="https://api.tmhub.jp/1/login"  

#ログイン時情報
$loginParams = @{
email="自分のメールアドレス"
password="パスワード"
}

#チームID
$teamId="xxxxx"

#イベント情報 日付 start
$dateStart="2023-12-01T15:00:00.000Z"
#イベント情報 日付 end
$dateEnd="2023-12-25T14:59:59.999Z"
#イベント情報 抽出上限
$selectLimit="999"

#イベント情報の取得
$reqURL="https://api.tmhub.jp/1/teams/${teanId}/events?from=${dateStart}&to=${dateEnd}&limit=${selectLimit}"

#ログインセッション確立 必須
$ls=Invoke-WebRequest $loginUrl -body $loginParams -SessionVariable loginSession

#WebRequest
$req=Invoke-WebRequest -Method Get $reqURL -WebSession $loginSession

#contentのJsonに変換⇒Jsonから変換
#文字化けしないし、コンソールとかからは見やすい
#$req.Content | ConvertFrom-Json | ConvertTo-Json

$eventJson=$req.Content | ConvertFrom-Json

#イベント情報
$eventJson 

